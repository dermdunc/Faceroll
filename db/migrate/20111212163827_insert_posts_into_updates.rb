class InsertPostsIntoUpdates < ActiveRecord::Migration
  def up
    select_rows("SELECT id, user_id, created_at FROM posts").each do |row|
      insert("INSERT INTO updates(content_type, conten_id, user_id, created_at) VALUES('Post', '#{row[0]}', '#{row[1]}', '#{row[2]}')")
    end
  ...
  def down
    ...
    select_rows("SELECT conten_id, user_id FROM updates").each do |row|
